...
### 中位数median
 中位数可将样本中的数值集合划分为相等的两部分，
 实数 ${\displaystyle x_{1},x_{2},\dots,x_{T}}$按大小顺序（顺序，降序皆可）排列为 ${\displaystyle x'_{1},x'_{2},\dots ,x'_{n}}$, 则中位数为

${\displaystyle \mathrm {Q} ={\begin{cases}x'_{\frac {T+1}{2}},&{\mbox{if }}T{\mbox{是奇数.}}\\{\frac {1}{2}}(x'_{\frac {n}{2}}+x'_{{\frac {n}{2}}+1}),&{\mbox{if }}T{\mbox{ 是偶数.}}\end{cases}}}$
...
### 几何平均数
几何平均数 是指的观测值的乘积之观测值个数方根，即 $${\displaystyle (x_{1}\times x_{2}\times x_{3}\ldots \times x_{n})^{\frac {1}{T}}}$$
### 调和平均数 
观测值个数除以观测值倒数的总和，即
$${\displaystyle {\frac {n}{{\frac {1}{x_{1}}}+{\frac {1}{x_{2}}}+...+{\frac {1}{x_{T}}}}}}$$ 
...
![Pasted Graphic 3.tiff](resources/C36B8F3168E4133ACFF377398270003B.jpg =591x361)
...
{\displaystyle Kurtosis={1 \over nD^{2}}\sum _{i=1}^{n}{(x_{i}-{\bar {x}})^{4}}-3} $$
...
![IMAGE](resources/61628C5723AC92145A119715CAA82253.jpg =824x352)

偏度的计算公式是：
$Skewness={\frac {m_{3}}{{m_{2}}^{3/2}}}={\frac {{\tfrac {1}{n}}\sum _{i=1}^{n}(x_{i}-{\overline {x}})^{3}}{\left({\tfrac {1}{n}}\sum _{i=1}^{n}(x_{i}-{\overline {x}})^{2}\right)^{3/2}}}$
...
$$\hat \rho_k = \frac{\hat \gamma_k }{\hat \gamma_0 }=\frac{\sum\limits_{t=1}^{n-1} (Y_t- \bar Y )(Y_{t+k}-\bar Y) }{\sum\limits_{t=1}^{n} (Y_t- \bar Y_t )^2}$$

![IMAGE](resources/69B54D676DB2C72B51F9309858721E2D.jpg =872x742)

![IMAGE](resources/1FD4B82FE9C4E0B87DC50D46D5AB1EF9.jpg =855x754)
...
$$\hat p(k) = \frac{ \rho_k -\alpha_{1}\rho_{k-1}-\dots -\alpha_{k-1}\rho_{1}}{ 1 -\alpha_{1}\rho_{1}-\dots -\alpha_{k-1}\rho_{k-1}}$$

![IMAGE](resources/69B54D676DB2C72B51F9309858721E2D.jpg =872x742)


![IMAGE](resources/AD0E1A8740C525047D5927493B07A445.jpg =874x759)

## 自回归模型(Autoregressive Models)
$AR(p)$模型，假设单条时序数据的取值为$ X_1 ,\dots,X_t $，

在自回归模型中，t时刻的取值$X_t$,可以用前面一个长度为$p$的窗口的数据来刻画，也就是：

$X_t = \sum\limits_{i=1}^{p} \phi_i X_{t-i}+ c+\epsilon_t$

该模型中的回归系数$a_1,\dots,a_p,c$需要从历史数据中训练得到，可以作为该时间序列的一个特征。
...
对arma模型进行参数估计，得到$\phi_i, i = 1,2,\dots,p$,
和$\varepsilon_j, j=1,2,\dots,q$,可作为时序特征。
...
二维的GMM(K=2)图示
![IMAGE](resources/A1FACD90620D37E7D9F45350824C11CF.jpg =1074x608)

二维的GMM(K=1)图示
![IMAGE](resources/0D15FB33E9EAF11A206C22CFF3615FF1.jpg =1096x628)
...
$f_w = \sum\limits_{t=0}^{T-1} X_{t+1} e^{(-i2\pi/N)tn} \qquad \omega = 0,\dots,T-1$


![IMAGE](resources/FB4E26285ABF67822EE43AD6D4B1BEB4.jpg =1084x650)
...
![IMAGE](resources/C44AEC5C0CB9B39559E6D0E06060BD86.jpg =1508x1088)
...
![IMAGE](resources/4E4CA84B4A2058E5D6A1602526D26436.jpg =1582x1142)
...
,&{\mbox{if }}s =1,2,\dots,q\\{
}\sum\limits_{i=1}^p\phi_i\hat x_{t+s-i|t},
&{\mbox{if }}s = q+1,q+2 \dots\end{cases}}}$
其中
$\hat X_{\tau|t} = X_{\tau}, \tau<=t$

下图是arima预测的效果图
![IMAGE](resources/07B165BBD36B9F7BE39EF5C44FC28A77.jpg =916x398)
